package betx.application.filter;
...
public class CustomAuthorizationFilter extends OncePerRequestFilter {
    ...
    private final ArrayList<String> permittedPaths = new ArrayList<>() {{
        add("/admin/register");
        add("/admin/auth");
        ...
        if (permittedPaths.contains(request.getServletPath())) {
            ...
            if (authorisationHeader != null && authorisationHeader.startsWith("Bearer ")) {
                try {
                    ...
                    filterChain.doFilter(request, response);
                    log.info("Granted authorization for user with email: " + username + " for roles: " + authorities);
                } catch (Exception e) {
                    ...
                    error.put("error_message", e.getMessage().substring(e.getMessage().indexOf(':') + 2));
